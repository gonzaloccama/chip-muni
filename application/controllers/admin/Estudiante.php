...

{
...

	{
		...
		$data['csvlink'] = base_url() . 'admin/estudiante/export/csv';

		$data['pdflink'] = base_url() . 'admin/estudiante/export/pdf';

		$data['per_page'] = isset($_GET['per_page']) && in_array($_GET['per_page'], $per_page_arr) ? $_GET['per_page'] : "124";
		...
		$config["base_url"] = base_url() . "admin/estudiante/index/$rel_field/$rel_id";

		$total_row = $this->estudiante->getCount('estudiante', $searchBy, $searchValue, $rel_arr);

		$config["first_url"] = base_url() . "admin/estudiante/index/$rel_field/$rel_id" . '?' . $_SERVER['QUERY_STRING'];
		...
		if (!$this->ion_auth->logged_in()) {
			...
			$this->load->view('admin/estudiante/manage', $data);
...

	{
		...
		$this->form_validation->set_rules('dni_familia', 'Dni_familia Name', 'trim');
...
//
			...
			//$dni_familia_ = $saveData['dni_familia'];
			...
			redirect("admin/estudiante/index/$rel_field/$rel_id");
...

	{
		...
		if (isset($id) and !empty($id)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->load->view('admin/estudiante/view', $data);

			} else {

				redirect('admin/estudiante/view');
...

	{
		...
		if (isset($id) and !empty($id)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->load->view('admin/estudiante/edit', $data);
